...
def test_get_sensor_keys_returns_dotted_names(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    db = tmp_path / "data.db"
    now = time.time()
    _make_db(db, [
...
def test_get_sensor_keys_deduplicates(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    db = tmp_path / "data.db"
    now = time.time()
    _make_db(db, [
...
def test_cleanup_db_retention_deletes_old_rows(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    db = tmp_path / "data.db"
    now = time.time()
...
def test_cleanup_db_retention_zero_keeps_all(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    db = tmp_path / "data.db"
    now = time.time()
    old = now - 200 * 86400
    _make_db(db, [
...
def test_cleanup_db_size_deletes_oldest_10pct(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    db = tmp_path / "data.db"
    now = time.time()
...
def test_cleanup_db_noop_when_no_db_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_get_db_stats_returns_expected_shape(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    db = tmp_path / "data.db"
    now = time.time()
    _make_db(db, [
...
def test_get_latest_filters_by_hive_id(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    db = tmp_path / "data.db"
    now = time.time()
    _make_db(db, [
...
def test_ensure_hive_id_column_is_idempotent(tmp_path: Path) -> None:
    ...
    conn.execute("CREATE TABLE measurements (id INTEGER, sensor_name TEXT, key TEXT, value REAL, timestamp REAL)")
    conn.commit()
